...
/*
 * lock() and noschedlock() are kept as macros
 * to preserve the caller pc.
 */
int
xlock(Lock *l, int nosched)
{
	int i;
	uintptr pc;
...
/*
 * unlock() and noschedunlock() are kept as macros
 * to preserve the caller pc.
 */
void
xunlock(Lock *l, int nosched)
{
#ifdef LOCKCYCLES
	uvlong x;
	cycles(&x);
