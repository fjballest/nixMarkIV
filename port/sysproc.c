...
sysrfork(Ar0* ar0, va_list list)
{
	...
	if((flag&RFPROC) == 0) {
		...
		if(flag & (RFNAMEG|RFCNAMEG)) {
			opg = up->pgrp;
			if(flag & RFNAMEG)
				up->pgrp = duppgrp(opg, up->slash);
			else
				up->pgrp = duppgrp(nil, up->slash);
			/* inherit noattach */
			up->pgrp->noattach = opg->noattach;
			up->pgrp->vers = opg->vers+1;
			closepgrp(opg);
		}
		if(flag & RFNOMNT)
			up->pgrp->noattach = 1;
	...
	/* make sure the parent dot is up to date wrt its ns,
	 * or if it spawns many times we will resolve dot each time.
	 */
	updatedot();
	p->dot = up->dot;
	incref(p->dot);
	...
	/*
	 * The text is shared, the stack is copied, and
	 * forkseg takes care of flushing mmu entries for others.
	 */
	for(i = 0; i < NSEG; i++)
		if(up->seg[i])
			forkseg(p, i, n);
	...
	if(flag & (RFNAMEG|RFCNAMEG)) {
		if(flag & RFNAMEG)
			p->pgrp = duppgrp(up->pgrp, p->slash);
		else{
			p->pgrp = duppgrp(nil, p->slash);
			p->pgrp->vers = up->pgrp->vers+1;
		}
...
sysexec(Ar0* ar0, va_list list)
{
	...
	Segment *s, *img;
	int argc, i, n, nargs;
	char *a, *args, **argv, elem[sizeof(up->genbuf)], *p;
	...
	chan = namec(file->s, Aopen, OEXEC, 0);
	if(waserror()){
		cclose(chan);
		nexterror();
	}
	...
	textmin = ROUNDUP(UTZERO+hdrsz+textsz, UPGSZ);
	textlim = UTROUND(textmin);
	datalim = ROUNDUP(textlim+datasz, UPGSZ);
	bsslim = ROUNDUP(textlim+datasz+bsssz, UPGSZ);
	...
	up->seg[ESEG] = newseg(SG_STACK, TSTKTOP-USTKSIZE, TSTKTOP, nil, UPGSHFT);
	prefaultseg(up->seg[ESEG]);
	/*
	...
	*--argv = nil;
	...
	/* attachseg returns a locked cache image */

	s = newseg(SG_TEXT|SG_RONLY, UTZERO, textmin, chan, UPGSHFT);
	img = s;
	up->seg[TSEG] = s;
	s->flushme = 1;
	s->fstart = 0;
	...
	/* Data (including BSS) Shared and attached by hand */
	s = newseg(SG_DATA, textlim, bsslim, nil, UPGSHFT);
	up->seg[DSEG] = s;
	...
	s->top = USTKTOP;
	relocateseg(s, USTKTOP-USTKSIZE, TSTKTOP-USTKSIZE);

	prefaultseg(up->seg[TSEG]);
	prefaultseg(up->seg[SSEG]);
