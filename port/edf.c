 ...
 * Does edf need this?
 * Edf shouldn't mess up with the std. scheduler. It should manage its own
 * queue(s) and leave the best effort scheduler alone.
 * The only thing the std. scheduler has to do is ingore edf processes and
 * call edf entry points.
 */
static Proc*
