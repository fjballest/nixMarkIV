...
#include	"../port/edf.h"
#include	"tos.h"
#include	<trace.h>	/* can't include ptrace.h; compat */
#include	"ureg.h"
...
static Lock tlock;
static int tproduced, tconsumed, topens;
...
procopen(Chan *c, int omode)
{
	...
	case Qns:
		if(omode != OREAD)
			error(Eperm);
		c->aux = malloc(sizeof(Nsread));
		if(c->aux)
			memset(c->aux, 0, sizeof(Nsread));
		break;
...
procfdprint(Chan *c, int fd, int w, char *s, int ns)
{
	...
	if(w == 0)
		w = procqidwidth(c);
	n = snprint(s, ns, "%3d %.2s %C %4ud (%.16llux %*lud %.2ux) %5ld %8lld %N\n",
		fd,
		&"r w rw"[(c->mode&3)<<1],
		c->dev->dc, c->devno,
...
procwrite(Chan *c, void *va, long n, vlong off)
{
	...
	case Qmem:
		...
		if(!iseve())
			error(Eperm);
		if(offset < PTR2UINT(etext) || offset+n >=PTR2UINT(end))
			error("kernel address out of range");
		memmove(UINT2PTR(offset), va, n);
		break;
...
proctext(Chan *c, Proc *p)
{
	...
	if(tc == nil)
		error(Eprocdied);
	if(nref == 1 || (tc->flag&COPEN) == 0 || tc->mode!=OREAD){
		cclose(tc);
		error(Eprocdied);
	}
	return tc;
...
procctlreq(Proc *p, char *va, int n)
{
	...
	case CMdebug:
		...
		if(cb->nf != 5)
			error("dump name amode omode perm");
		Chan *c;
		if(waserror()){
			print("cnamec %s: %s\n", cb->f[1], up->errstr);
			nexterror();
		}
		c = namec(cb->f[1], atoi(cb->f[2]), atoi(cb->f[3]), strtoul(cb->f[4], nil, 0));
		if(c == nil)
			error("cname returns nil");
		print("%s -> %N mnt %#p ref %d mode %d fid %d\n",
			cb->f[1], c->path, c->mnt, c->ref, c->mode, c->fid);
		poperror();
		if(c != nil)
			cclose(c);
		break;
...
txt2data(Proc *p, Segment *s)
{
	...
	ps = newseg(SG_DATA, s->base, s->top, nil, s->pgszlg2);
	ps->src = s->src;
	if(ps->src == nil)
		ps->src = s;
	incref(ps->src);
...
data2txt(Segment *s)
{
	...
	ps = newseg(SG_TEXT, s->base, s->top, nil, s->pgszlg2);
	ps->src = s->src;
	if(ps->src == nil)
		ps = s;
	incref(ps->src);
