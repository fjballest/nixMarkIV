...
openmode(int omode)
{
	omode &= ~(OTRUNC|OCEXEC|ORCLOSE|OBEHIND);
	if(omode > OEXEC)
		error(Ebadarg);
	if(omode == OEXEC)
		return OREAD;
...
syspipe(Ar0* ar0, va_list list)
{
	...
	c[0] = walk(c0, &datastr[0], 1);
	c[1] = walk(c0, &datastr[1], 1);
...
void
sysfdflush(Ar0* ar0, va_list list)
{
	int fd;
	Chan *c;
...
{
	uchar *p;
	int dirlen, rest, nname, nd;
	long l;
	uchar *buf, *e;
	Dir d;
	Namec narg;
	char *cname, *cnameend, *name;

	/* no mounts under # names */
	if(c->path->nels > 0 && *c->path->els[0] == '#')
		return n;
	p = op;
	buf = smalloc(4096);
	cname = (char*)buf+(3*1024);
	if(waserror()){
		free(buf);
		nexterror();
	}
	/*
	 * Perhaps we should use paths as the interface for namec/nameop
	 */
	cnameend = seprint(cname, cname+1024, "%N", c->path);
	if(cnameend[-1] == '/')
		cnameend--;
	for(e=&p[n]; p+BIT16SZ<e; p+=dirlen){
		*cnameend = 0;
		...
		if(cnameend-cname + nname + 1 >= 1024)
			error("mountfix: name too long");
		*cnameend = '/';
		memmove(cnameend+1, name, nname);
		cnameend[nname+1] = 0;
		narg.nd = 3*1024;
		narg.d = buf;
		if(waserror())
			continue;
		nameop(cname, Astat, &narg);
		if(narg.ismtpt == 0){
			poperror();
			continue;
		}
		nd = narg.nd;
		narg.nd = dirsetname(name, nname, narg.d, nd, 3*1024);
		if(narg.nd == BIT16SZ)
			error("dirsetname");
		poperror();
		/*
...
bindmount(int ismount, int fd, int afd, char* arg0, char* arg1, int flag, char* spec)
{
	...
	if(ismount){
		...
		if(waserror())
			error(Ebadspec);
		specnm = validnamedup(validaddr(spec, 1, 0), 1);
		poperror();
		bogus.spec = specnm->s;
...
syscreate(Ar0* ar0, va_list list)
{
	...
	openmode(omode & ~(OEXCL|OBEHIND));	/* error check only */
...
sysremove(Ar0* ar0, va_list list)
{
	 ...
	 */
	aname = va_arg(list, char*);
	nameop(validaddr(aname, 1, 0), Aremove, nil);
...
syswstat(Ar0* ar0, va_list list)
{
	...
	narg.d = p;
	narg.nd = n;
	nameop(validaddr(aname, 1, 0), Awstat, &narg);
	ar0->l = narg.nd;
}
