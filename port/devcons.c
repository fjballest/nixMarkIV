...
typedef char *(*Sumf)(char*,char*,void*);
...
void
addsummary(char *(*f)(char*,char*, void*), void *a)
{
	int i;

	lock(&summarylk);
	for(i = 0; i < nelem(summary); i++){
		if(summary[i].f == nil){
			summary[i].f = f;
			summary[i].a = a;
			unlock(&summarylk);
...
void
addttescape(uint c, void (*f)(void*), void *a)
{
	if(c >= nelem(ttescape))
		panic("addttescape '%c'", c);
	if(ttescape[c].f != nil)
		panic("addttescape: dup '%c'", c);
	ttescape[c].f = f;
	ttescape[c].a = a;
}
...
consread(Chan *c, void *buf, long n, vlong off)
{
	...
	case Qalloc:
		...
		bp = b;
		*b = 0;
		for(i = nelem(summary)-1; i >= 0; i--)
			if(summary[i].f != nil)
				b = summary[i].f(b, bp+READSTR, summary[i].a);
		n = readstr(offset, buf, n, bp);
		free(bp);
		return n;
