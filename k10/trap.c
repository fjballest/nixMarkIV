...
static int faultspanic = 1;	/* debug */
...
/* debug: to panic on user faults due to errors if enabled */
static int isfault[32] = {
[IdtDF]	1,
[IdtTS] 1,
[IdtUD] 1,
[IdtNP] 1,
...
trap(Ureg* ureg)
{
	...
	if(ctl = vctl[vno]){
		...
		if(faultspanic && isfault[vno])
			panic("faulted: pid %d: %s", up?up->pid:0, buf);
...
dumpstackwithureg(Ureg* ureg)
{
	...
	for(l = (uintptr)&l; l < estack; l += sizeof(uintptr)){
		...
		|| ((uintptr)&l < v && v < estack) || estack-l < 3*KiB){
