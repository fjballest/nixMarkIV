...
/*
 * PGSZ PGSHFT correspond to the smallest MMU page size
 * The system may be using larger sizes.
 * (eg., user segments use UPGSZ).
 */
#define PGSZ		(4*KiB)			/* page size */
#define PGSHFT		12			/* log(PGSZ) */
#define PTSZ		(4*KiB)			/* page table page size */
#define PTSHFT		9			/* log(PTSZ) */
#define UPGSZ		(16*KiB)		/* user page size */
#define UPGSHFT		14			/* log(UPGSZ) */

#define MACHSZ		(4*KiB)			/* Mach+stack size */
#define MACHMAX		32			/* max. number of cpus */
#define MACHSTKSZ	(6*(4*KiB))		/* Mach stack size */

#define KSTACK		(32*1024)		/* Size of Proc kernel stack */
...
#define USTKTOP		ROUNDDN(0x00007ffffffff000ull, UPGSZ)
...
/*
 * GAK kernel memory.
 * mmuinit assumes it's at least 4M.
 * Using less than 16M is likely to make the kernel fail
 * (eg. procalloc is about 7M for 2000 procs).
 */
#define TMFM		(16*MiB)
