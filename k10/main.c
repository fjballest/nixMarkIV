...
//		i8250console("0");
	...
	up->pgrp->mnt = setslash(up->pgrp->mnt, up->slash);
...
userinit(void)
{
	 ...
	 */
	s = newseg(SG_STACK, USTKTOP-USTKSIZE, USTKTOP, nil, UPGSHFT);
	p->seg[SSEG] = s;
	pg = newpage(UPGSZ, s->color, 1, USTKTOP-UPGSZ);
	segpage(s, pg);
	k = kmap(pg);
	bootargs(VA(k));
	kunmap(k);
	 ...
	 * Text
	 */
	s = newseg(SG_TEXT, UTZERO, UTZERO+UPGSZ, nil, UPGSHFT);
	s->flushme++;
	p->seg[TSEG] = s;
	pg = newpage(UPGSZ, s->color, 1, UTZERO);
	/* might have to PG_TXTFLUSH pg */
